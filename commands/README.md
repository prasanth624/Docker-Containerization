...
```bash
[prasanth@localhost ~]# docker stop my-nginx
my-nginx
```
...
```bash
[prasanth@localhost ~]# docker start my-nginx
my-nginx
```
...
```bash
[prasanth@localhost ~]# docker restart my-nginx
my-nginx
```

> Restarts the container (stop + start)
...
```bash
[prasanth@localhost ~]# docker logs my-nginx
```

> Displays logs generated by the container (useful for debugging)
...
```bash
[prasanth@localhost ~]# docker exec -it my-nginx /bin/bash
```
...
```bash
[prasanth@localhost ~]# docker rm cce590127ebf
cce590127ebf
```

> Deletes a stopped or exited container
...
```bash
[prasanth@localhost ~]# docker rmi nginx
Untagged: nginx:latest
Deleted: sha256:248d2326f351
```
...
```bash
[prasanth@localhost ~]# docker volume ls
```

> Displays Docker volumes used for persistent data
...
```bash
[prasanth@localhost ~]# docker info
```

> Provides detailed information about the Docker daemon, storage, and runtime
